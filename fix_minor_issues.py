...
CSS_REPLACEMENTS = {
    '/Users/borjafernandezangulo/projects/sonograft/src/style.css': [
        (r'backdrop-filter:([^;]+);(?![\s\n]*-webkit-backdrop-filter)', r'-webkit-backdrop-filter:\1;\n  backdrop-filter:\1;'),
        (r'-webkit-mask:([^;]+);', r'-webkit-mask:\1;\n  mask:\1;'),
        (r'mask-image:([^;]+);\s*-webkit-mask-image:([^;]+);', r'-webkit-mask-image:\2;\n  mask-image:\1;'),
        (r'backdrop-filter:([^;]+);\s*-webkit-backdrop-filter:([^;]+);', r'-webkit-backdrop-filter:\2;\n  backdrop-filter:\1;')
    ],
    ...
    '/Users/borjafernandezangulo/cortex/saas/dashboard.html': [
        (r'backdrop-filter:([^;]+);', r'-webkit-backdrop-filter:\1;\n            backdrop-filter:\1;')
    ]
}
...
for p in tsconfig_fixes:
    if os.path.exists(p):
        ...
        if '"forceConsistentCasingInFileNames"' not in c:
            c = re.sub(r'"compilerOptions":\s*\{', '"compilerOptions": {\n    "forceConsistentCasingInFileNames": true,', c)
            with open(p, 'w') as f:
                f.write(c)
