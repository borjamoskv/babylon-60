...
class R01ExperimentHarness:
    ...
    def _evaluate_pair(self, name: str, g1: List[float], g2: List[float]) -> Tuple[float, float, float]:
        if not g1 or not g2:
            return 1.0, 0.0, 0.0
    ...
    def evaluate_h0(self):
        ...
        sys.stdout.write(f"Preservation Rule: PresenceScore Formula {R01_SUCCESS_CRITERIA['presence_formula']} [FROZEN]\n")
        
        if n_total < R01_SUCCESS_CRITERIA["min_sample_size"]:
            sys.stdout.write(f"[WARNING] Muestra insuficiente ({n_total}/{R01_SUCCESS_CRITERIA['min_sample_size']}). Falsación abortada.\n")
            sys.stdout.write("="*50 + "\n\n")
            return
        ...
        if (p2 < R01_SUCCESS_CRITERIA["target_p_value"] and 
            d2 > R01_SUCCESS_CRITERIA["target_cohens_d"] and 
            i2 >= R01_SUCCESS_CRITERIA["min_improvement_pct"]):
            sys.stdout.write("VEREDICTO: [H0 RECHAZADA] Efecto estructural detectado en brazo Saliency.\n")
            sys.stdout.write("NOTA: Se requiere replicación independiente (Cohortes = 2) para asentar evidencia.\n")
        else:
...
if __name__ == "__main__":
    ...
    if args.simulate:
        ...
        for i in range(600):
            ...
            harness.log_observation(user_id, group, score, random.uniform(10, 300), random.uniform(0, 1), random.uniform(60, 1200))
