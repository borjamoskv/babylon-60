...
# Inicializar modelo
try:
    local_llm = ChatOllama(model=model_name, temperature=0.0, base_url="http://localhost:11434")
...
def run_pipeline():
    ...
    messages = [
        SystemMessage(
            content="Eres KETER_LOCAL, un agente autónomo C5-REAL de coding. Cero narrativa. Solo respuestas estructurales o código."
        ),
        HumanMessage(
            content="Escribe un comando bash de una sola línea para listar todos los archivos Python en el directorio actual y cuenta cuántos hay."
        ),
    ]
