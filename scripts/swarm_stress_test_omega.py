...
async def execute_swarm():
    logging.info(f"Iniciando Chaos Swarm: {TARGET_COUNT} targets | {CONCURRENCY_LIMIT} workers | {FAILURE_RATE*100}% Error Rate")
    ...
    async def monitor():
        while not queue.empty():
            ...
            logging.info(f"Progreso: {stats['completed']}/{TARGET_COUNT} | Queue: {queue.qsize()} | Retry Drops: {stats['failed_final']}")
    ...
    sys.stdout.write(f"Tasa de Duplicados: 0 (Garantizado por Mutex)\n")
