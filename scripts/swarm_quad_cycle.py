...
async def main():
    ...
    print("-" * 60)
    print(f"◈ Cycle 1 (MCTS Fuzzing)      -> {vulns_found} critical vulnerabilities/paths discovered.")
    print(f"◈ Cycle 2 (SOTA Distillation) -> {mechanisms_extracted} architectural mechanisms crystallized.")
    print(f"◈ Cycle 3 (Deep-Scraping)     -> {leads_generated} high-fidelity B2B leads injected into pipeline.")
    print(f"◈ Cycle 4 (Ouroboros LEA-Ω)   -> {debt_purged} tokens of technical debt purged from workspace.")
    print("-" * 60)
