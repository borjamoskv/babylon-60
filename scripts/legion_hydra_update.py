...
class SwarmEngine:
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    ...
                    if any(x in err_str for x in ["quota", "429", "limit", "exhausted", "503"]):
                        print(
                            f"      {c(YELLOW, f'⚠️ API Rate Limit/Error. Retrying in {wait:.1f}s...')}"
                        )
    ...
    async def process_file(self, path: Path, file_type: str, index: int, total: int):
        ...
        print(
            f"{c(CYAN, f'[{index:3d}/{total}]')} 🐝 Despachando {c(BOLD, file_type)}: {DIM}{path.relative_to(Path.home())}{RESET}"
        )
