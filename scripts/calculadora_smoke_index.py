...
GRAPH_FILE = Path("data/mafia_ai/structural_graph.graphml")
OUTPUT_CSV = Path("data/reputation_graph/concentration_index_report.csv")
OUTPUT_CSV_ALT = Path("data/reputation_graph/smoke_index_report.csv")
OUTPUT_CSV_MAFIA = Path("data/mafia_ai/smoke_index_report.csv")
...
def calculate_smoke_index():
    ...
    for path in [OUTPUT_CSV, OUTPUT_CSV_ALT, OUTPUT_CSV_MAFIA]:
    ...
    print(f"[*] Smoke Index Reports exportados a {OUTPUT_CSV}, {OUTPUT_CSV_ALT} y {OUTPUT_CSV_MAFIA}")
