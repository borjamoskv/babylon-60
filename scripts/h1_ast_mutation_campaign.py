...
class Mutator(ast.NodeTransformer):
    ...
    def visit_Compare(self, node):
        ...
        def action():
            for i, op in enumerate(node.ops):
                if isinstance(op, ast.Eq): node.ops[i] = ast.NotEq()
                elif isinstance(op, ast.NotEq): node.ops[i] = ast.Eq()
                elif isinstance(op, ast.Is): node.ops[i] = ast.IsNot()
                elif isinstance(op, ast.IsNot): node.ops[i] = ast.Is()
                elif isinstance(op, ast.In): node.ops[i] = ast.NotIn()
                elif isinstance(op, ast.NotIn): node.ops[i] = ast.In()
                elif isinstance(op, ast.Lt): node.ops[i] = ast.GtE()
                elif isinstance(op, ast.LtE): node.ops[i] = ast.Gt()
                elif isinstance(op, ast.Gt): node.ops[i] = ast.LtE()
                elif isinstance(op, ast.GtE): node.ops[i] = ast.Lt()
    ...
    def visit_Return(self, node):
        self.generic_visit(node)
        def action():
            if node.value and isinstance(node.value, ast.Constant) and isinstance(node.value.value, bool):
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        logger.info("Usage: python h1_ast_mutation_campaign.py <target_directory> [more_targets...]")
        sys.exit(1)
    ...
    report = {
        "CI_Mutation_Report": {
            ...
            "Evaluation": {
                "Status": status,
                "Passed": mutation_score >= thresholds["General"]
            }
        }
    }
