...
logger = logging.getLogger("inject_mac_control")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para MAC-CONTROL-Ω...")
    ...
    md_path = "AUTODIDACT_MAC-CONTROL.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(MAC-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 110 # Elevated PID representation for Daemon control
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "MAC_CONTROL_ROOT", 0.0)

    for i, match in enumerate(matches):
        p_id, p_name, p_app = [m.strip() for m in match]
        axiom_claim = f"{p_id}: {p_name}"
        constructive_proof = f"Intervención de sistema C5-REAL: {p_app}. DAG vinculado por HoTT engine en la capa base de OS."
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "MAC_CONTROL_OS_LEVEL", 0.1)
    
    logger.info("Inyección criptográfica OS completada.")
