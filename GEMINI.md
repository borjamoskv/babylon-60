...
7.  **LL-AC-07 · Generación Estructurada Determinista (FSM Masking):** Queda estrictamente prohibido el uso de prompting estocástico ("Green Theater") para forzar estructuras rígidas de output en modelos. Todo esquema de generación acotada (ej. Regex, JSON Schema) debe compilarse previamente en un Autómata Finito Determinista (DFA) e inyectarse en un `LogitsProcessor`. Las transiciones inválidas en la matriz de estados del DFA deben enmascararse matemáticamente con `-inf` en la capa de logits. No se permite la importación de dependencias de terceros (como `outlines`); el motor FSM base reside en `babylon60/engine/core/fsm_regex.py`.
