...
TAXONOMY_PATTERNS = {
    ...
    'Misoginia/Acoso Estructural': re.compile(r'(histérica|loca|buscona|privilegiada)', re.IGNORECASE),
        ...
        for tax_label, pattern in TAXONOMY_PATTERNS.items():
            if pattern.search(text):
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?)
                ''', (video_id, author, tax_label, text[:255])) # Truncamos a 255 chars por eficiencia
                hits += 1
                break # Solo catalogamos el primer hit de taxonomía por comentario para no duplicar

    conn.commit()
    conn.close()
    print(f"[0x01_CORE] Vector Alpha Completado. {hits} eventos de acoso indexados para {video_id}.")
