...
class UltramapSubstrate:
    ...
    def __init__(self, capacity=10000):
        ...
        if HAS_EVOLUTION_LEDGER:
            ...
            try:
                ...
                logger.info(f"EVO-LEDGER Active. Head: {self._evolution_ledger.head_hash[:12]}… Seq: {self._evolution_ledger.sequence}")
    ...
    def update_control_vector(self, agent_idx: int, queue_depth: float, error_rate: float, causal_entropy: float, cpu_load: float, *, source: str = "substrate") -> bool:
        ...
        """
        if self._rs is not None:
            success = self._rs.update_control_vector(agent_idx, queue_depth, error_rate, causal_entropy, cpu_load)
            if success and self._evolution_ledger is not None:
                self._emit_ledger_event(agent_idx, None, queue_depth, error_rate, causal_entropy, cpu_load, source)
            return success
        ...
        if self._evolution_ledger is not None:
            self._emit_ledger_event(agent_idx, vector_before, queue_depth, error_rate, causal_entropy, cpu_load, source)
