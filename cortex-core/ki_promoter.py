...
PROJECT_CONTEXT = {'cortex-persist': {'tags': {'cortex', 'ai-agent', 'persistence', 'infrastructure'}, 'domain': 'cortex'}, 'cortex': {'tags': {'cortex', 'ai-agent', 'research', 'infrastructure'}, 'domain': 'cortex'}, 'bounty': {'tags': {'bounty', 'immunefi', 'security', 'web3', 'critical', 'high'}, 'domain': 'security'}, 'music': {'tags': {'music', 'sonic', 'audio', 'mastering'}, 'domain': 'multimedia'}, 'systemforge': {'tags': {'infrastructure', 'macos', 'system'}, 'domain': 'infrastructure'}}
...
def detect_context() -> dict:
    ...
    try:
        result = subprocess.run(['git', 'rev-parse', '--show-toplevel'], capture_output=True, text=True, timeout=3)
...
def score_ki(meta: dict, meta_path: Path, context: dict) -> ScoredKI:
    ...
    return ScoredKI(name=meta_path.parent.name, path=meta_path, score=round(score, 4), tag_score=round(tag_score, 3), domain_score=round(domain_score, 3), freshness_score=round(freshness_score, 3), domain=ki_domain, tags=list(ki_tags)[:5], summary_preview=preview)

def promote_ki(ki: ScoredKI) -> None:
    """
...
def main() -> None:
    ...
    logger.info(f"{'=' * 70}")
    logger.info('KI CONTEXT PROMOTER')
    logger.info(f"{'=' * 70}")
    logger.info(f"  Project:  {context.get('project', '?')}")
    logger.info(f"  Domain:   {context['domain']}")
    logger.info(f"  Tags:     {', '.join(sorted(context['tags']))}")
    logger.info(f'  Top-N:    {top_n}')
    logger.info(f"  Mode:     {('DRY-RUN' if dry_run else 'PROMOTE')}")
    logger.info()
    scored = []
    ...
    logger.info(f"{'#':<4} {'Score':<8} {'Tag':<6} {'Dom':<6} {'Fresh':<7} {'Domain':<15} {'Name'}")
    logger.info(f"{'─' * 4} {'─' * 8} {'─' * 6} {'─' * 6} {'─' * 7} {'─' * 15} {'─' * 40}")
    for i, ki in enumerate(scored[:top_n]):
        marker = '→' if not dry_run else ' '
        logger.info(f'{marker}{i + 1:<3} {ki.score:<8.4f} {ki.tag_score:<6.3f} {ki.domain_score:<6.3f} {ki.freshness_score:<7.3f} {ki.domain:<15} {ki.name[:40]}')
    if len(scored) > top_n:
    ...
    if not dry_run:
        ...
        for ki in scored[:top_n]:
            try:
                ...
                logger.info(f'  ERROR promoting {ki.name}: {e}')
        logger.info(f'\n  ✓ Promoted {promoted}/{top_n} KIs')
        logger.info('  → These will be injected in the next conversation')
    else:
        logger.info(f'\n  → Run without --dry-run to promote top-{top_n}')
    logger.info('\n  Bottom 5 (lowest relevance):')
    for ki in scored[-5:]:
        logger.info(f'    {ki.score:.4f}  {ki.domain:<15} {ki.name[:50]}')
