...
async def list_skills() -> Dict[str, Any]:
    ...
    return {
        "count": len(skills),
        "skills": [
            {"name": name, "path": str(path), "has_skill_md": True}
...
async def cloud_status() -> Dict[str, Any]:
    ...
    return {
        "clouds": {
            "aws": {"region": "eu-west-1", "status": "healthy", "cluster": "cortex-sovereign-production-eks"},
            "gcp": {"region": "europe-west1", "status": "healthy", "cluster": "cortex-sovereign-production-gke"},
            "azure": {"location": "West Europe", "status": "healthy", "cluster": "cortex-sovereign-production-aks"},
        },
        "total_replicas": 9,
        "healthy_replicas": 9,
    }
