...
class LedgerAuditMixin:
    ...
    async def audit_integrity_async(self, tenant_id: str | None=None) -> dict:
        ...
        async with self._get_conn_proxy() as conn:
            started_at = now_iso()
            cursor = await conn.execute("SELECT id, COALESCE(tenant_id, 'default'), project, action, detail, prev_hash, hash, timestamp FROM transactions ORDER BY id")
                ...
                if in_scope:
                    ...
                    if computed_v3 == h:
                        ...
                        if prev != expected_prev_global:
                            violations.append({'id': tid, 'type': 'CHAIN_BREAK', 'expected': expected_prev, 'legacy_expected': expected_prev_global})
                    else:
            ...
            if facts_table_exists:
                ...
                if tenant_id is None:
                    ...
                    fact_cursor = await conn.execute("SELECT tx_id, content, hash, COALESCE(tenant_id, 'default') FROM facts WHERE tenant_id = ?", (tenant_id,))
                while True:
                    ...
                    if f_tx_id is not None:
                        active_facts_by_tx[f_tx_id] = {'content': f_content, 'hash': f_hash, 'tenant_id': f_tenant_id}
                ...
                for f_tx_id, info in list(active_facts_by_tx.items()):
                    try:
                        ...
                        if computed_hash != info['hash']:
                            violations.append({'id': f_tx_id, 'type': 'FACT_HASH_MISMATCH', 'stored_hash': info['hash'], 'computed_hash': computed_hash})
                    except Exception as e:
                ...
                for store_tid, expected_hash in store_txs_to_verify.items():
                    if store_tid in active_facts_by_tx:
                        ...
                        if fact_hash != expected_hash:
                            violations.append({'id': store_tid, 'type': 'FACT_MUTATION_DETECTED', 'expected_hash': expected_hash, 'fact_hash': fact_hash})
            ...
            if tenant_id is None:
                ...
                cursor = await conn.execute("SELECT COALESCE(tenant_id, 'default'), root_hash, tx_start_id, tx_end_id FROM merkle_roots WHERE tenant_id = ?", (tenant_id,))
            roots = list(await cursor.fetchall())
            for root_tenant_id, stored_root, start, end in roots:
                if root_tenant_id == '__global__':
                    c = await conn.execute('SELECT hash FROM transactions WHERE id >= ? AND id <= ? ORDER BY id', (start, end))
                else:
                    c = await conn.execute('SELECT hash FROM transactions WHERE tenant_id = ? AND id >= ? AND id <= ? ORDER BY id', (root_tenant_id, start, end))
            ...
            await conn.execute('INSERT INTO integrity_checks (check_type, status, details, started_at, completed_at) VALUES (?, ?, ?, ?, ?)', ('full' if tenant_id is None else 'tenant', status, json.dumps(violations), started_at, now_iso()))
            await conn.commit()
