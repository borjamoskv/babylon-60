...
if TYPE_CHECKING:
    from cortex.experimental.extensions.interfaces.engine import EngineProtocol
...
class FactManager:
    ...
    async def graph(self, *args, **kwargs) -> Any:
        ...
        import cortex.experimental.graph

        async with self.engine.session() as conn:
            return await cortex.experimental.graph.get_graph(conn, *args, **kwargs)
    ...
    async def query_entity(self, *args, **kwargs) -> Any:
        ...
        async with self.engine.session() as conn:
            return await cortex.experimental.graph.query_entity(conn, *args, **kwargs)

    async def find_path(self, *args, **kwargs) -> Any:
        import cortex.experimental.graph

        async with self.engine.session() as conn:
            return await cortex.experimental.graph.find_path(conn, *args, **kwargs)

    async def get_context_subgraph(self, *args, **kwargs) -> Any:
        import cortex.experimental.graph

        async with self.engine.session() as conn:
            return await cortex.experimental.graph.get_context_subgraph(conn, *args, **kwargs)
