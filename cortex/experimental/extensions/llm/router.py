...
from cortex.experimental.extensions.llm._cascade import CascadeManager, classify_tier
from cortex.experimental.extensions.llm._hedging import HedgedRequestStrategy
from cortex.experimental.extensions.llm._models import (
    BaseProvider,
    CascadeEvent,
    CascadeTier,
    CortexPrompt,
...
from cortex.experimental.extensions.llm._telemetry import CascadeTelemetry
from cortex.utils.result import Err, Ok, Result

logger = logging.getLogger("cortex.experimental.extensions.llm.router")
...
class CortexLLMRouter:
    ...
    def _ordered_fallbacks(self, prompt: CortexPrompt | IntentProfile) -> list[BaseProvider]:
        ...
        from cortex.experimental.extensions.llm._models import ReasoningMode
    ...
    async def route(
        ...
        if not provider_hint and prompt.system_instruction:
            ...
            try:
                from cortex.experimental.extensions.swarm.kv_prefix_registry import get_kv_registry
    ...
    def optimal_provider_order(
        ...
        *,
        ...
        try:
            from cortex.experimental.extensions.llm._presets import providers_for_intent
    ...
    def frontier_order(intent: str) -> list[tuple[str, str]]:
        ...
        try:
            from cortex.experimental.extensions.llm._presets import frontier_providers
