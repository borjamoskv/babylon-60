...
def action(trajectory: List[Particle], state_vec: np.ndarray, meta: Any, field: FailureField) -> float:
    S = 0.0
    for t in trajectory:
        L = lagrangian(t, state_vec, meta, field)
        dt = t.original_stats.runtime_mean if getattr(t.original_stats, 'runtime_mean', 0) > 0 else 1.0
...
def sample_trajectories(tasks: List[Particle], horizon: int = 5, num_samples: int = 50) -> List[List[Particle]]:
    if not tasks:
        return []
...
def select_next(tasks: List[Particle], state_vec: np.ndarray, field: FailureField, meta: Any, horizon: int = 5, epsilon: float = 0.1) -> List[Particle]:
    best_path = None
