    ...
    async def spawn_timeline(self, base_tenant: str = "default") -> str:
        ...
        async with aiosqlite.connect(self.db_path, timeout=10) as conn:
            ...
            for r in rows:
                ...
                insert_data.append((
                    new_id, new_tenant, r[1], r[2], new_lineage, r[4], r[5], r[6]
                ))
        ...
        logger.info(f"[Bifurcation] Nueva línea temporal '{new_tenant}' forjada a partir de '{base_tenant}'.")
        return new_tenant
    ...
    async def evaluate_multiverse(self, window_seconds: int = 3600) -> list[dict[str, Any]]:
        ...
        for state in multiverse_state:
            ...
            if state["mode"] == "chaotic_irreversible" or state["eb"] <= 0:
                logger.critical(f"[Bifurcation] Timeline '{t_id}' colapsada (Muerte Térmica). Exergy: 0.0")
            elif state["mode"] == "coherence_lock":
                logger.warning(f"[Bifurcation] Timeline '{t_id}' en Cuarentena Causal (CF: {state['cf']:.2f}).")
            else:
                logger.info(f"[Bifurcation] Timeline '{t_id}' estable. Exergy: {state['exergy']:.2f}")
