...
def evaluate_retention(dry_run=True):
    ...
    for node in risk_nodes:
        logger.info(f"[TAINT: RISK_OF_CHURN] {node['name']} ({node['email']}) - {node['days_active']} días sin actividad. Aperturas: {node['opens']}.")
        ...
        if not dry_run:
            ...
            cursor.execute("UPDATE audience SET status = 'churn_mitigated' WHERE email = ?", (node['email'],))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--execute', action='store_true', help='Desactiva el dry-run y ejecuta mutaciones de estado/envíos')
