...
## Universal Sovereign Autopoiesis (AX-046)

As per the root manifesto, ALL agents interacting with the Engine are inherently **Sovereign**. 
- Code dynamically generated by agents (JIT Autopoiesis) holds the exact same ontological weight as human-authored code.
- Any SAGA-mutation originating from an agent's synthesized logic must be intercepted and validated by `store_validation.py` just like any other generative output.
- If a Sovereign agent writes and executes code locally, it MUST route through `worktree_isolation` or `code_smith` before touching the core Engine state.
