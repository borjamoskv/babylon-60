    ...
    res.update(
        {
            ...
            "created_at": row[14],
            "updated_at": row[15],
            "is_tombstoned": bool(row[16]),
            "is_quarantined": bool(row[17]),
            "quadrant": row[20] or "ACTIVE",
            "storage_tier": row[21] or "HOT",
            "exergy_score": _to_float(row[22], 1.0),
            "category": row[23] or "general",
            "semantic_status": row[24],
            "semantic_error": row[25],
            "parent_id": row[26],
            "relation_type": row[27],
            "yield_score": _to_float(row[28], 1.0),
            "tags_raw": row[29] if len(row) > 29 else None,
        }
    )
