...
try:
    ...
    Z3_AVAILABLE = False
    logger.warning(
        "z3-solver module not found. Falling back to simple heuristic contradiction engine."
    )
...
class DeterministicSolver:
    ...
    def prove_contradiction(fact_a: dict[str, Any], fact_b: dict[str, Any]) -> bool:
        ...
        if meta_a and meta_b and meta_a.get("var_name") == meta_b.get("var_name"):
            ...
            if "min" in meta_a:
                solver.add(x >= int(meta_a["min"]))
            if "max" in meta_a:
                solver.add(x <= int(meta_a["max"]))

            # Add constraints for B
            if "min" in meta_b:
                solver.add(x >= int(meta_b["min"]))
            if "max" in meta_b:
                solver.add(x <= int(meta_b["max"]))
            ...
            if result == z3.unsat:
                logger.critical(
                    f"[Z3_SOLVER] UNSAT: Mathematical contradiction proved on variable '{var_name}'."
                )
                return True
            elif result == z3.unknown:
                logger.critical(
                    f"[Z3_SOLVER] UNKNOWN: Solver timed out or failed on variable '{var_name}'. Enforcing containment."
                )
                return True
