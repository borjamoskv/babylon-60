...
class EpistemicArbitrator:
    ...
    def route(self, context: ExecutionContext) -> ModelType:
        ...
        if context.task_type in self.SEMANTIC_TASKS:
            logger.info("[ARBITRATOR] Task '%s' -> 3.1 Pro (Semantic Layer Rule)", context.task_type)
            return "gemini-3.1-pro"
            
        # 2. Heuristic Thresholds (Entropy & Topology)
        if context.kl_instability > self.KL_INSTABILITY_THRESHOLD:
            logger.info("[ARBITRATOR] KL Instability (%.2f) > %.2f -> 3.1 Pro", 
                        context.kl_instability, self.KL_INSTABILITY_THRESHOLD)
            return "gemini-3.1-pro"
            
        if context.entropy_score > self.ENTROPY_THRESHOLD:
            logger.info("[ARBITRATOR] Entropy (%.2f) > %.2f -> 3.1 Pro", 
                        context.entropy_score, self.ENTROPY_THRESHOLD)
            return "gemini-3.1-pro"
            
        if context.ast_complexity > self.COMPLEXITY_THRESHOLD:
            logger.info("[ARBITRATOR] AST Complexity (%d) > %d -> 3.1 Pro", 
                        context.ast_complexity, self.COMPLEXITY_THRESHOLD)
            return "gemini-3.1-pro"
