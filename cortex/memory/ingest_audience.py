...
def ingest_active_subscribers(csv_path: str):
    ...
    with open(csv_path, encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            last_sign_in_at = (datetime.now() - timedelta(days=1)).isoformat() if opens > 0 else None
            ...
            try:
                ...
                ''', (email, name, created_at, last_sign_in_at, opens, clicks, days_active, source, cluster, raw_meta, taint))
                count += 1
...
def ingest_global_enriched(csv_path: str):
    ...
    with open(csv_path, encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            taint = _generate_taint_hash(email, created_at)
            raw_meta = json.dumps({"subdomain": subdomain, "source_url": source_url, "target_type": "volume"})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingesta Soberana de Audiencia para CORTEX-Sidecar")
