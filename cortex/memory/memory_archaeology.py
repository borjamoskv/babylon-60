...
class MemoryArchaeologist:
    ...
    def _fetch_active_facts(self, project: str, tenant_id: str) -> dict[str, dict[str, Any]]:
        ...
        cursor.execute(
            """
            SELECT id, tenant_id, content, parent_decision_id
            FROM facts
    ...
    def _extract_vectors(
        ...
        project: str,
        tenant_id: str,
        l3_map: dict[str, dict[str, Any]],
    ) -> tuple[list[dict[str, Any]], np.ndarray | None]:
        l2_conn = self.engine.memory._l2._get_conn()
        c2 = l2_conn.cursor()
        c2.execute(
            "SELECT m.id, v.embedding FROM facts_meta m "
    ...
    async def _apply_db_updates(
        ...
        # Tombstone old ones
        c3.execute(
            f"UPDATE facts SET is_tombstoned = 1, valid_until = ? "
        ...
        if primary_parent_id:
            c3.execute(
                "UPDATE facts SET parent_decision_id = ? WHERE id = ? AND tenant_id = ?",
        ...
        cl2.execute(
            ...
            f"(SELECT rowid FROM facts_meta WHERE tenant_id = ? AND id IN ({placeholders}))",
        ...
        c3.execute(
            f"UPDATE facts SET parent_decision_id = ? WHERE tenant_id = ? "
            f"AND parent_decision_id IN ({placeholders})",
            [new_fact_id, tenant_id] + old_ids,
        )
        cl2 = l2_conn.cursor()
        cl2.execute(
            f"UPDATE facts_meta SET parent_decision_id = ? WHERE tenant_id = ? "
            f"AND parent_decision_id IN ({placeholders})",
            [new_fact_id, tenant_id] + old_ids,
        )
