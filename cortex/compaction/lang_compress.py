...
_ES_MARKERS: frozenset[str] = frozenset(
    "que los las del para con por una como más pero fue son está todo también cuando sobre entre "
    "después desde donde cada puede este esta estos estas ese esa esos esas aquel aquella muy hay "
    "tiene hace siendo otro otra otros otras mismo misma hacia sino según durante aunque porque "
    "mientras además así mejor algún ningún parte decisión error proyecto memoria sesión".split()
)

# Common Basque markers (agglutinative morphology = high token tax)
_EU_MARKERS: frozenset[str] = frozenset(
    "eta bat da ere hau bere dago dira gara beste baino hori hura baina nola zer zein nahi egin "
    "izan dut dugu duzu dute zuen gure nire haien bertan gainera beraz orduan orain gero lehen "
    "aurrera atzera goian behean bakoitzak guztiak batzuk gehiago gutxiago azkar poliki ondo gaizki "
    "berri zahar".split()
)
