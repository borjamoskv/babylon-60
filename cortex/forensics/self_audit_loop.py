...
def compile_and_run_tests(vulnerabilities):
    ...
    for vuln in vulnerabilities:
        ...
        try:
            ...
            process = subprocess.run([sys.executable, temp_path], capture_output=True, text=True)
...
def main():
    ...
    if has_blind_spots:
        console.print(
            Panel(
                "[bold red]TOPOLOGÍA ROTA. BUCLE DETENIDO POR FALLO EN LA CONFIANZA DEL ORÁCULO EXTERNO.[/bold red]"
            )
        )
        sys.exit(1)
    else:
