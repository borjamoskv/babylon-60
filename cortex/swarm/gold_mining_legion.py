...
def extract_market_alpha(
    ...
    for wave_idx, wave_size in enumerate(wave_sizes):
        ...
        legion.yield_tensor = np.random.lognormal(mean=2.0, sigma=1.5, size=wave_size).astype(np.float32)
        ...
        if (wave_idx + 1) % 10 == 0 or wave_idx == num_waves - 1:
            logger.info("  Processed Wave %03d/%03d | Slashed: %d noise | Retained: %d ELITE leads", 
                        wave_idx + 1, num_waves, slashed, survivors)
