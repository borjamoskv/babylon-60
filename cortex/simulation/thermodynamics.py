...
class MemoryFrictionEngine:
    ...
    def compute_friction(self, hop_distance: int, semantic_similarity: float, temporal_gap: float) -> float:
...
class MemoryEnergyField:
    ...
    """
    @staticmethod
    def compute_energy(trajectories: List[MemoryTrajectory], drift: float, base_budget: float = 100.0) -> ThermodynamicState:
        if not trajectories:
            return ThermodynamicState(global_temperature=0.0, phase="SOLID", total_energy=0.0, energy_budget=base_budget)
