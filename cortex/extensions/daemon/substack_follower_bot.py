...
async def run_automata():
    ...
    try:
        async with async_playwright() as p:
            ...
            if not contexts:
                logger.error("No se encontraron contextos de navegador. Abre Chrome con el flag --remote-debugging-port=9222")
                return
            ...
            if not target_page:
                target_page = pages[0]
                logger.warning(f"No se encontró una URL explícita de Substack. Operando sobre la pestaña activa: {target_page.url}")
                ...
                for i, button in enumerate(buttons_to_click):
                    try:
                        ...
                        if await button.is_visible() and await button.is_enabled():
                            ...
                            logger.info(f" -> Nodo {i+1} ignorado (Botón oculto o deshabilitado).")
