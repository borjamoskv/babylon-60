...
class NightShiftDaemon:
    ...
    def execute_purge(self) -> str:
        ...
        episodic_memory = "\n".join(raw_events)
        logger.info(f"Extracted {len(raw_events)} exergic events. Total length: {len(episodic_memory)} chars.")
        ...
        logger.info(f"NightShift complete. Episodic noise purged. Axiom crystallized at {axiom_file}")
