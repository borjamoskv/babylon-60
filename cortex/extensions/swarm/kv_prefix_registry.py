...
class PrefixSlot:
    ...
    created_at: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())  # noqa: TID251
...
class KVPrefixRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._prefix_providers: dict[str, set[str]] = {}  # prefix_hash → set of cache_keys
    ...
    def check_cache_affinity(
        self, system_prompt: str, episodic_context: list[dict] | None = None
    ) -> list[str]:
        """Returns provider names that have an active cache for this exact prompt hash.
        O(1) Check using dict comprehension and active lazy eviction of expired TTL slots.
        """
        prefix_hash = hash_prompt(system_prompt, episodic_context)
        keys = tuple(self._prefix_providers.get(prefix_hash, set()))
        providers = set()
        now = time.time()
        ...
        for key in keys:
            slot = self._slots.get(key)
            if slot is None or slot.prefix_hash != prefix_hash:
        ...
        # JIT Active Garbage Collection (Memory Leak Mitigation P1)
        for e_key in expired_keys:
            self._slots.pop(e_key, None)
            provider_set = self._prefix_providers.get(prefix_hash)
            if provider_set and e_key in provider_set:
                provider_set.remove(e_key)
                if not provider_set:
                    self._prefix_providers.pop(prefix_hash, None)

        return list(providers)

    async def wait_or_acquire_prefill(
        self, system_prompt: str, episodic_context: list[dict] | None = None
    ) -> bool:
        """
        ...
        if prefix_hash in self._prefill_locks:
            ...
            try:
                # Deadlock Mitigation (P0): Avoid forever-hangs if Leader tasks crash abruptly.
                await asyncio.wait_for(event.wait(), timeout=15.0)
            except asyncio.TimeoutError:
                pass  # Timeout reached, follower should proceed independently
            return False
