...
class DryRunEngine:
    ...
    async def simulate(self, operations: List[MigrationOperation]) -> DryRunResult:
        ...
        for op in operations:
            ...
            if not op.sql_down:
                warnings.append(f"Irreversible operation detected: no rollback_sql for {op.op} on {op.table}")
        ...
        guards["rollback_check"] = GuardResult(
            ...
            details="Some operations lack rollback SQL" if any(not op.sql_down for op in operations) else "All operations reversible",
            severity="warning"
        )
