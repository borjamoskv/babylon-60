...
class AsyncSignalBus:
    ...
    async def stats(self, *, tenant_id: str = "default") -> dict:
        ...
        cursor = await self._conn.execute(
            "SELECT event_type, COUNT(*) FROM signals WHERE tenant_id = ?"
        ...
        cursor = await self._conn.execute(
            "SELECT source, COUNT(*) FROM signals WHERE tenant_id = ?"
    ...
    async def gc(self, max_age_days: int = 30, *, tenant_id: str = "default") -> int:
        await self.ensure_table()
...
class SignalBus:
    ...
    def gc(self, max_age_days: int = 30, *, tenant_id: str = "default") -> int:
        self.ensure_table()
        cutoff = (datetime.now(timezone.utc) - timedelta(days=max_age_days)).isoformat()
        cursor = self._conn.execute(
            """DELETE FROM signals
