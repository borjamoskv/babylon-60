...
from cortex.extensions.mejoralo.constants import ESCALATION_ITER_L2, ESCALATION_ITER_L3, HARD_ITERATION_CAP, MIN_PROGRESS, PYTEST_TIMEOUT_SECONDS, STAGNATION_LIMIT
from cortex.extensions.mejoralo.deps import sort_by_topological_order
from cortex.extensions.mejoralo.heal_prompts import get_files_per_iteration as _get_files_per_iteration
...
async def _heal_file_async(file_path: Path, findings: list[str], level: int=1, iteration: int=0, engine: MejoraloEngine | None=None, project: str | None=None) -> str | None:
    ...
    return await swarm.refactor_file(file_path, findings, iteration=iteration, engine=engine, project=project)
...
def _apply_and_verify(top_file_rel: str, new_code: str, path: str | Path, level: int, iteration: int, console: Any, current_score: int, engine: MejoraloEngine | None=None, project: str | None=None) -> bool:
    ...
    if not _run_functional_inquisitor(new_code, original_code, top_file_rel, console, engine, project, abs_path):
        return False
    abs_path.write_text(new_code)
    _apply_aesthetic_formatting(abs_path, console)
    if not _run_delta_testing(top_file_rel, path, original_code, abs_path, console, engine, project, level):
        return False
    return _commit_healed_file(abs_path, path, top_file_rel, level, iteration, current_score, console, complexity_delta=complexity_delta, engine=engine, project=project)

def _run_functional_inquisitor(new_code: str, original_code: str, top_file_rel: str, console: Any, engine: MejoraloEngine | None, project: str | None, abs_path: Path) -> bool:
    if abs_path.suffix != '.py':
        return True
    try:
        ...
        if deleted:
            console.print(f"  [bold red]🚫 Inquisidor: Eliminación de funciones públicas detectada ({', '.join(deleted)}). BLOQUED.[/]")
...
def _apply_aesthetic_formatting(abs_path: Path, console: Any) -> None:
    ...
    subprocess.run([sys.executable, '-m', 'ruff', 'check', '--fix', str(abs_path)], capture_output=True)

def _run_delta_testing(top_file_rel: str, path: str | Path, original_code: str, abs_path: Path, console: Any, engine: MejoraloEngine | None, project: str | None, level: int=1) -> bool:
    ...
    try:
        res = subprocess.run(pytest_cmd, cwd=path, capture_output=True, text=True, timeout=PYTEST_TIMEOUT_SECONDS)
        if res.returncode != 0:
            ...
            if engine and project:
                ...
                if level >= 3:
                    console.print(f'  [bold red]☠️ L3 CIRCUIT BREAKER: {top_file_rel} marcado como TAINTED. Requiere ariadne-arch-omega.[/]')
                    mark_file_tainted(top_file_rel, project, engine)
            abs_path.write_text(original_code)
            return False
        return True
    except subprocess.TimeoutExpired as e:
        console.print(f'  [bold red]⏳ Timeout en {top_file_rel} tras {PYTEST_TIMEOUT_SECONDS}s! Rollback.[/]')
        if engine and project:
...
def _commit_healed_file(abs_path: Path, path: str | Path, top_file_rel: str, level: int, iteration: int, current_score: int, console: Any, complexity_delta: int=0, engine: MejoraloEngine | None=None, project: str | None=None) -> bool:
    try:
        ...
        subprocess.run(['git', 'commit', '-m', commit_msg, '--author', 'CORTEX MEJORAlo Auto-Heal <cortex@moskv.1>'], cwd=path, capture_output=True)
        if engine and project:
            hours = calculate_chronos_yield(files_touched=1, codepaths_affected=max(1, complexity_delta), runtime_ms=0, cyclomatic_complexity_delta=complexity_delta)
            try:
                engine.record_session(project=project, score_before=current_score, score_after=current_score, actions=[f'Healed {top_file_rel} (L{level}, iter {iteration})', f'CHRONOS-1 yield: {hours}h saved'])
...
def heal_project(project: str, path: str | Path, target_score: int, scan_result: ScanResult, engine: MejoraloEngine | None=None) -> bool:
    ...
    while current_result.score < target_score and iteration < HARD_ITERATION_CAP:
        ...
        _print_iteration_header(console, iteration, level, project, current_result.score, target_score, stagnation_count)
        iteration_success, current_result = _run_healing_iteration(project, path, level, iteration, console, current_result, healed_files, engine=engine)
        any_success = any_success or iteration_success
        score_history.append(current_result.score)
        stagnation_count = _check_stagnation(console, score_history, stagnation_count, iteration_success)
        if stagnation_count >= STAGNATION_LIMIT * 3 or current_result.score >= target_score:
            ...
            break
    return _report_final_state(console, current_result, target_score, iteration, score_history, any_success)

def _run_healing_iteration(project: str, path: str | Path, level: int, iteration: int, console: Any, current_result: ScanResult, healed_files: set[str], engine: MejoraloEngine | None=None) -> tuple[bool, ScanResult]:
    ...
    async def _run_generations():
        ...
        for f, iss in targets:
            result = await _heal_file_async(Path(path).resolve() / f, iss, level=level, iteration=iteration, engine=engine, project=project)
    ...
    try:
        ...
        if loop.is_running():
            with concurrent.futures.ThreadPoolExecutor() as executor:
                generation_results = executor.submit(lambda: asyncio.run(_run_generations())).result()
        else:
    ...
    for (top_file_rel, _), new_code in zip(targets, generation_results, strict=True):
        if is_file_tainted(top_file_rel, project, engine):
            console.print(f'  [bold red]☠️ {top_file_rel} está TAINTED. Requiere ariadne-arch-omega. Saltando.[/]')
            continue
        if new_code and _apply_and_verify(top_file_rel, new_code, path, level, iteration, console, current_result.score, engine=engine, project=project):
...
def _print_iteration_header(console: Any, iteration: int, level: int, project: str, current_score: int, target_score: int, stagnation_count: int) -> None:
...
def _check_stagnation(console: Any, score_history: list[int], stagnation_count: int, iteration_success: bool) -> int:
    """Evaluate whether the last iteration made progress."""
    if len(score_history) >= 2:
        delta = score_history[-1] - score_history[-2]
        if delta < MIN_PROGRESS:
            stagnation_count += 1
            console.print(f'  [yellow]⚠️ Progreso insuficiente (Δ{delta:+d}). Estancamiento: {stagnation_count}/{STAGNATION_LIMIT}[/]')
        else:
            stagnation_count = 0
...
def _report_final_state(console: Any, current_result: ScanResult, target_score: int, iteration: int, score_history: list[int], any_success: bool) -> bool:
...
def heal_proj(project: str, path: str | Path, target_score: int, scan_result: ScanResult, engine: MejoraloEngine | None=None) -> bool:
