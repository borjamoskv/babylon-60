...
from cortex.extensions.perception.base import MIN_EVENTS_FOR_INFERENCE, BehavioralSnapshot, FileEvent
...
def infer_behavior(events: list[FileEvent]) -> BehavioralSnapshot:
    ...
    if total < MIN_EVENTS_FOR_INFERENCE:
        return BehavioralSnapshot(intent='unknown', emotion='neutral', confidence='C1', project=_dominant_project(events), event_count=total, window_seconds=_window_duration(events), top_files=_top_files(events, 5), summary=f'Insufficient activity ({total} events)', timestamp=now_iso())
    for condition, intent, emotion, confidence in _INTENT_RULES:
        if condition(stats):
            project = _dominant_project(events)
            return BehavioralSnapshot(intent=intent, emotion=emotion, confidence=confidence, project=project, event_count=total, window_seconds=_window_duration(events), top_files=_top_files(events, 5), summary=_generate_summary(intent, emotion, total, project), timestamp=now_iso())
    return BehavioralSnapshot(intent='active', emotion='neutral', confidence='C2', project=_dominant_project(events), event_count=total, window_seconds=_window_duration(events), top_files=_top_files(events, 5), summary=f'General activity: {total} file events', timestamp=now_iso())
...
def _generate_summary(intent: str, emotion: str, count: int, project: str | None) -> str:
    ...
    intents = {'debugging': 'Debugging/testing session detected', 'setup': 'Infrastructure/configuration work', 'deep_work': 'Focused deep work session', 'experimenting': 'Experimentation cycle (create-delete-iterate)', 'frustrated_iteration': 'Iterating on same file repeatedly (possibly stuck)', 'documenting': 'Documentation pass', 'refactoring': 'Multi-file refactoring session'}
