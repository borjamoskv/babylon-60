...
from cortex.compliance.eu_ai_act import AIDecisionReport
...
def export_bundle(tenant_id: str, output: str) -> None:
    ...
    async def _export():
        ...
        console.print(f"[bold blue]Initiating EU AI Act Compliance Export for tenant:[/bold blue] {tenant_id}")
        ...
        async with aiosqlite.connect(db_path) as conn:
            ...
            try:
                ...
                console.print(f"  [green]✓[/green] Exported {len(ledger_data)} cryptographically linked ledger entries.")
        ...
        if pk_path.exists(): pk_path.unlink()
        if bundle_dir.exists(): bundle_dir.rmdir()
        
        console.print(f"[bold green]Compliance Bundle successfully exported to:[/bold green] {output_path.absolute()}")
