...
from cortex.cli.notebooklm_data import _PROJECT_DOMAIN, DIGEST_FILE, DOMAINS_DIR, NOTEBOOKLM_DIR, _detect_cloud_sync, _format_fact_obj, _get_engine_active_facts, _get_entities_and_relations, _run_async, _sovereign_signature
console = Console()
...
@click.option('--output', '-o', type=click.Path(), default=str(DIGEST_FILE), help='Output file path')
def digest_cmd(output: str) -> None:
    ...
    async def _digest() -> None:
        ...
        console.print(Panel(f'[green]✅ Master Digest generado (CLEAR_TEXT)[/green]\n  Archivo: {output}\n  Palabras: {word_count:,}\n  {limit_msg}', title='📓 NotebookLM Digest', border_style='green'))
    _run_async(_digest())

@notebooklm_cmds.command('fragment')
@click.option('--output-dir', '-o', type=click.Path(), default=str(DOMAINS_DIR), help='Output directory for domain fragments')
...
@click.option('--drive-path', type=click.Path(), default=None, help='Google Drive folder path (auto-detected if not set)')
@click.option('--mode', type=click.Choice(['digest', 'domains', 'both']), default='both', help='What to sync')
def sync_cmd(drive_path: str | None, mode: str) -> None:
    ...
    for f in target.glob('*.md'):
        ...
        if os.path.getmtime(f) < cutoff and f.name not in synced_names:
            f.unlink()
            cleaned += 1
    console.print(Panel(f"[green]✅ Sincronizados {len(synced_files)} archivos a {provider_name}[/green]\n  Destino: {target}\n  {('Limpiados ' + str(cleaned) + ' archivos antiguos' if cleaned else '')}\n\n  📋 NotebookLM debería detectarlos automáticamente\n     si Drive está conectado como fuente.", title='🔄 Google Drive Sync', border_style='green'))
    ...
    async def _ingest() -> None:
        ...
        async with SovereignLLM() as llm:
            for file_path in target.glob('*.md'):
                if file_path.name.startswith('cortex-master') or file_path.name.startswith('cortex-') or file_path.name == DIGEST_FILE.name:
                    continue
                ...
                if len(content) > 50000:
                    console.print(f'[yellow]⚠️  Archivo demasiado grande, saltando: {file_path.name}[/yellow]')
                    continue
                res = await llm.generate(content, system=system_prompt, intent=IntentProfile.EPISODIC_PROCESSING)
                if res.ok:
                    try:
                        ...
                        if isinstance(items, list):
                            for it in items:
                                if 'content' in it and 'project' in it:
                                    ...
                                    extracted_facts.append({'project': it['project'], 'content': it['content'], 'fact_type': it.get('fact_type', 'knowledge'), 'confidence': it.get('confidence', 'C3'), 'source': 'notebooklm:sync_daemon', 'meta': meta_dict})
        ...
        if extracted_facts:
            ...
            try:
                ...
                console.print(Panel(f'[green]✅ Ouroboros Loop Completado[/green]\nHechos asimilados: {len(ids)}\nArchivos procesados: {len(newly_processed)}', title='🧠 CORTEX Ingestion', border_style='green'))
            finally:
                await engine.close()
