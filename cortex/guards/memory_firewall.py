...
class MemoryFirewallGuard:
    ...
    def validate_fact(self, content: str, source: str, confidence: str, meta: dict[str, Any] | None = None) -> bool:
        """
        ...
        if clean_conf in self._confidence_rank:
            if self._confidence_rank[clean_conf] > self._confidence_rank.get(self.min_confidence, 3):
            ...
            logger.warning("Unrecognized confidence format '%s', bypassing threshold check.", confidence)
        ...
        if self.require_taint:
            ...
            if len(parts) >= 5 and parts[0] == "taint":
                ...
                if public_key_b64:
                    is_valid = ZKSwarmIdentity.verify_payload(content, public_key_b64, signature_b64)
                    if not is_valid:
                        raise ValueError("[P0] Memory Firewall: CORTEX-TAINT signature verification failed.")
            else:
