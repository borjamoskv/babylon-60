...
class CTREGuard:
    ...
    @staticmethod
    def validate_commit(expected_hash: int, current_hash: int, target_x: float = 0.0, target_y: float = 0.0) -> Tuple[bool, int]:
        """
        ...
        if HAS_RUST_CTRE and hasattr(cortex_rs, 'ctre_atomic_commit'):
            ...
            try:
                ...
                if not success:
                    logger.warning(f"[CTRE] SAGA ABORT: Mutación detectada (TOCTOU). Epsilon={epsilon_us}µs")
                else:
                    logger.info(f"[CTRE] COMMIT SUCCESS: Isomorfismo estructural verificado en {epsilon_us}µs")
    ...
    def _python_fallback(expected_hash: int, current_hash: int) -> Tuple[bool, int]:
        ...
        if not success:
            logger.warning(f"[CTRE-PY] SAGA ABORT (GIL Warning): Mutación detectada. Epsilon={epsilon_us}µs")
        return success, epsilon_us
