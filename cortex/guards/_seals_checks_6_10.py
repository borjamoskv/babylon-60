...
from cortex.guards.sovereign_seals import check_gate_21_preservation, check_seal_8_dependency_impl, check_seal_9_compliance_impl
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Call):
                    if isinstance(node.func, ast.Attribute) and node.func.attr == 'sleep' and isinstance(node.func.value, ast.Name) and (node.func.value.id == 'time') or (isinstance(node.func, ast.Name) and node.func.id == 'sleep'):
...
async def check_seal_6_async_perf() -> GateResult:
    ...
    passed = True
    exclude = frozenset(['seals.py', 'reactor.py', 'antipatterns.py', '_scanner_visitors.py', 'registry.py', 'legion.py', 'legion_vectors.py', 'demo_swarm.py', 'demo_bicameral.py', 'network.py', 'fiat_oracle.py', 'mouse.py', 'dashboard_cmds.py', 'health_cmds.py', 'ouroboros_omega.py', 'oracle.py'])
    ...
    critical = [ROOT_DIR / 'cortex/llm/router.py', ROOT_DIR / 'cortex/llm/provider.py', ROOT_DIR / 'cortex/guards/seals.py']
