...
class XForensicGuard:
    """Forensic-Grade X Intelligence Guard (AX-033).
    ...
    async def check(
        ...
        *,
        ...
        # 3. Anomaly detection: Zero content but high metrics
        likes = meta.get("favorite_count", 0)
        if not content.strip() and likes > 100:
             raise ValueError(
                "[AX-033] Inconsistent X Fact: High engagement but empty content. Potential scrape failure."
            )

        # 4. Content length gate
        if len(content) > 10000: # X supports long posts, but 10k+ is suspicious for basic scraper
            raise ValueError(
                "[AX-033] X content exceeds safety threshold (10k chars)."
            )
