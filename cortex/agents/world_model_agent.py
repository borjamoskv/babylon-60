...
class WorldModelWebAgent:
    ...
    def execute_react_step(self, observation: str) -> dict[str, Any]:
        ...
        signature = ZKSwarmIdentity.sign_payload(
            content=content_str, private_key_b64=self.keys.private_key_b64
        )
        ...
        signed_proposal = {
            ...
            "fact_type": "decision",
            "content": content_str,
            "meta": {"agent_public_key": self.keys.public_key_b64, "zk_proof_signature": signature},
        }

        logger.info(
            f"[{self.agent_id}] Proposed decision mathematically signed. Payload ready for Ouroboros."
        )
