...
async def execute_batch_store(
    ...
    """Helper to batch store memories or facts, avoiding duplicate logic."""
    ids: list[int] = []
    errors: list[dict] = []
    for i, mem in enumerate(req.memories):
        try:
            ...
            logger.exception("Failed to batch store %s at index %d", item_label, i)
            errors.append({"index": i, "error": f"Failed to store {item_label}"})
