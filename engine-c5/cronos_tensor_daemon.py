...
def cronos_decay():
    ...
    while True:
        ...
        if iterations % 20 == 0:
            ...
            logging.info(f"T={int(elapsed)}s | Retention: e^{retention:.4f} | L2 Norm: {memory_magnitude:.2f} | Silicon Loop: {iterations}")
            
        time.sleep(1.0) # Throttle to prevent complete CPU starvation of other MOSKV processes

    logging.info(f"CRONOS Phase Completed. Cognitive hardware cycle closed. Total decay iterations: {iterations}.")
