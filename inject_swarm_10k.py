...
logger = logging.getLogger("inject_swarm_10k")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para SWARM-10K...")
    ...
    md_path = "AUTODIDACT_SWARM-10K.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(SWARM10K-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 100
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "SWARM10K_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "SWARM10K_LEAF", 0.1)
