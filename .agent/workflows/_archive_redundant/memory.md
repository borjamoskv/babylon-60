---
description: Ejecuta el protocolo MEMORIA para persistir el contexto de la sesión actual en BABYLON60 (v4 - Full Ontology)
---

# Workflow: MEMORIA (v4)

Protocolo optimizado de persistencia de contexto para BABYLON60 v4. Ejecuta estos pasos en orden:

// turbo-all

1. **Smart Check**: Comprueba si el snapshot global está fresco (< 1 hora).
   - Si `~/.babylon60/context-snapshot.md` tiene menos de 1 hora, **recomienda al usuario saltar la ejecución completa** a menos que quiera forzar un update.
   - Comando para verificar edad:

     ```bash
     find ~/.babylon60/context-snapshot.md -mmin -60 2>/dev/null
     ```

2. **Recall**: Carga el estado actual del proyecto desde BABYLON60:

   ```bash
   cd ~/babylon60 && .venv/bin/python -m babylon60.cli recall $(basename "$(pwd)") 2>/dev/null || echo "No prior context"
   ```

3. **Analyze**: Analiza la conversación actual y extrae la ontología completa de BABYLON60 v4:
...
4. **Store Batch**: Genera un JSON temporal con TODOS los facts y súblelos de golpe.
   ...
   - Ejecuta la carga batch:
     ...
     ```bash
     cd ~/babylon60 && .venv/bin/python -m babylon60.cli store-batch _facts.json
     rm _facts.json
     ```
...
6. **Export**: Exporta el snapshot global actualizado:
   ...
   ```bash
   cd ~/babylon60 && .venv/bin/python -m babylon60.cli export
   ```
...
> **Véase también:** `/recall` para cargar contexto, `/babylon60-recall` para boot rápido.
