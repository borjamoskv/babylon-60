...
def test_labyrinth_routing():
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are subjected to a thermodynamic exergy filter. Speak concisely."},
        ...
        output_msg(f"Proof: {{ Response: [{response.choices[0].message.content.strip()}], Latency: [{elapsed:.2f}s], Confidence: [C5-REAL] }}")
        sys.exit(0)
