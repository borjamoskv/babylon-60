...
logger = logging.getLogger("inject_resilience")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para RESILIENCE-Ω...")
    ...
    md_path = "AUTODIDACT_RESILIENCE.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(RSL-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 140 # Elevated PID representation for Self-Healing Engine
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "RESILIENCE_ROOT", 0.0)

    for i, match in enumerate(matches):
        p_id, p_name, p_app = [m.strip() for m in match]
        axiom_claim = f"{p_id}: {p_name}"
        constructive_proof = f"Mecanismo de Inmortalidad Lógica activado: {p_app}. Asegurado en la red b60."
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "SELF_HEALING_NODE", 0.1)
    
    logger.info("Inyección criptográfica de Resiliencia completada.")
