...
def test_repo_health_skill_resolves_from_event():
    ...
    event = EventV1(event_type="command", source="test", skill_id="repo_health", payload={"command": "check"})
...
def test_repo_health_trace_id_propagates():
    ...
    trace = "trace_abc123"
    event = EventV1(event_type="command", source="test", skill_id="repo_health", payload={"command": "check"}, trace_id=trace)
    skill = RepoHealthSkill()
    artifact = skill.execute(event)
...
def test_repo_health_unknown_command_returns_error_not_exception():
    ...
    event = EventV1(event_type="command", source="test", skill_id="repo_health", payload={"command": "made_up_command"})
    skill = RepoHealthSkill()
    artifact = skill.execute(event)
