...
async def test_epidemic_stress():
    ...
    for i in range(1, N_NODES):
        prev_node = nodes[i - 1]
        curr_node = nodes[i]
        await curr_node.node.register_peer(prev_node.node_id, f"127.0.0.1:{prev_node.node.bind_port}")
        # Make the connection bidirectional to speed up tests, though gossip handles discovery
        await prev_node.node.register_peer(curr_node.node_id, f"127.0.0.1:{curr_node.node.bind_port}")
