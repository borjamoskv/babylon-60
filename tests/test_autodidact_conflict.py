...
from cortex.extensions.skills.autodidact.synthesis import (
    execute_cognitive_synthesis,
)
    ...
    async with aiosqlite.connect(db_path) as conn:
        ...
        with patch("cortex.memory.encoder.LocalEmbedder") as mock_embedder_class:
            ...
            try:
                with (
                    patch(
                        "cortex.extensions.skills.autodidact.synthesis.get_memory_manager",
                        return_value=manager,
                    ),
                    patch(
                        "cortex.extensions.skills.autodidact.synthesis.distill_sovereign_memo",
                        new=AsyncMock(return_value=CRYSTAL_TERMINAL_X_AUTH),
                    ),
                    patch(
                        "cortex.extensions.skills.autodidact.synthesis.generate_cortex_embedding",
                        new=AsyncMock(return_value=[0.1] * 384),
                    ),
                ):
                    # 1. Establish a sovereign fact about 'Terminal-X'
                    res1 = await execute_cognitive_synthesis(
                        raw_data="[LOG] Terminal-X connection established", source="auth_monitor"
                    )
                    assert "filtered:conflict" not in res1, (
                        f"Unexpected conflict on first store: {res1}"
                    )
                ...
                events = await l3.get_session_events(
                    session_id="autodidact_knowledge", tenant_id="sovereign"
                )

                conflict_events = [
                    e for e in events if e.metadata.get("type") == "epistemic_conflict"
                ]
                assert len(conflict_events) >= 1, (
                    f"No conflict events in L3. All events: {[e.metadata for e in events]}"
                )
