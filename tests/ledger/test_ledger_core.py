...
def test_merkle_tree_odd_leaves():
    ...
    leaves3 = [hashlib.sha256(b"a").hexdigest(), hashlib.sha256(b"b").hexdigest(), hashlib.sha256(b"c").hexdigest()]
...
def test_sovereign_ledger_sync_ops(tmp_path):
    ...
    # Verify chain in DB
    cursor = conn.execute("SELECT prev_hash, hash FROM transactions WHERE tenant_id='t1' ORDER BY id")
