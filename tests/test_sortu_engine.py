...
class TestQuarantineSweep:
    def test_expired_skill_gets_quarantined(self, engine):
        ...
        future = datetime.fromtimestamp(time.monotonic(), tz=timezone.utc) + timedelta(days=10)
...
class TestTombstoneSweep:
    def test_quarantined_past_grace_gets_tombstoned(self, engine):
        ...
        # Quarantine it
        future = datetime.fromtimestamp(time.monotonic(), tz=timezone.utc) + timedelta(days=5)
        engine.quarantine_sweep(now=future)

        # Tombstone it (grace_days=7 from TTL expiration)
        far_future = datetime.fromtimestamp(time.monotonic(), tz=timezone.utc) + timedelta(days=30)
...
class TestPurgeSweep:
    def test_tombstoned_past_retention_gets_purged(self, engine):
        ...
        far = datetime.fromtimestamp(time.monotonic(), tz=timezone.utc) + timedelta(days=100)
