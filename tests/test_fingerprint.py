...
def _patch_scanner(mock_data: dict):
    ...
    scanner_patch = patch(
        "legacy_research.extensions.fingerprint.extractor.FingerprintScanner", autospec=True
    )
...
class TestFingerprintEmpty:
    ...
    async def test_empty_db_returns_null_state(self):
        engine = _make_engine()
        with patch("legacy_research.extensions.fingerprint.extractor.FingerprintScanner") as MockScanner:
            m = AsyncMock()
