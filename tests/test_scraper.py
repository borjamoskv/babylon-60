...
from cortex.experimental.extensions.scraper.extractors import (
    ExtractionError,
    HttpExtractor,
    html_to_markdown,
)
...
class TestHttpExtractor:
    ...
    async def test_successful_extraction(self):
        ...
        with patch(
            "cortex.experimental.extensions.scraper.extractors.httpx.AsyncClient", return_value=mock_client
        ):
...
class TestRobotsTxt:
    ...
    async def test_allowed_path(self):
        ...
        from cortex.experimental.extensions.scraper.extractors import check_robots_txt
...
class TestScraperEngine:
    ...
    async def test_cascade_first_strategy_succeeds(self):
        ...
        with patch.dict(
            "cortex.experimental.extensions.scraper.extractors.EXTRACTORS", {"http_fast": mock_extractor}
        ):
            with patch("cortex.experimental.extensions.scraper.extractors.CASCADE_ORDER", ["http_fast"]):
    ...
    async def test_cascade_fallback(self):
        ...
        with patch.dict("cortex.experimental.extensions.scraper.extractors.EXTRACTORS", extractors, clear=True):
            with patch("cortex.experimental.extensions.scraper.extractors.CASCADE_ORDER", cascade):
                result = await engine._cascade_extract("https://example.com", 15.0)
    ...
    async def test_all_strategies_fail(self):
        ...
        with patch.dict(
            "cortex.experimental.extensions.scraper.extractors.EXTRACTORS", {"http_fast": mock_ext}, clear=True
        ):
            with patch("cortex.experimental.extensions.scraper.extractors.CASCADE_ORDER", ["http_fast"]):
                result = await engine._cascade_extract("https://example.com", 15.0)
    ...
    async def test_deduplication_detection(self):
        ...
        with (
            patch.dict(
                "cortex.experimental.extensions.scraper.extractors.EXTRACTORS",
                {"http_fast": mock_ext},
                clear=True,
            ),
            patch("cortex.experimental.extensions.scraper.extractors.CASCADE_ORDER", ["http_fast"]),
            patch("cortex.experimental.extensions.scraper.extractors.check_robots_txt", return_value=True),
        ):
