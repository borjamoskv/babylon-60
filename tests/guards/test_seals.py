...
async def test_seal_1_happy(mock_cache):
    ...
    with patch("legacy_research.guards._seals_checks_1_5.arun_cmd", new_callable=AsyncMock) as mock_run:
...
async def test_seal_6_happy(mock_cache):
    ...
    with (
        patch(
            "legacy_research.guards._seals_checks_6_10._check_temperature_determinism",
            AsyncMock(return_value=[]),
        ),
        patch(
            "legacy_research.guards._seals_checks_6_10._check_latency_telemetry", AsyncMock(return_value=[])
        ),
    ):
...
async def test_seal_7_prompt_size_warn(tmp_path):
    ...
    with (
        patch("legacy_research.extensions.axioms.AXIOM_REGISTRY", mock_registry),
        patch("legacy_research.extensions.axioms.registry.enforced", return_value=mock_registry),
        patch("legacy_research.guards._seals_checks_6_10.ROOT_DIR", tmp_path),
    ):
...
async def test_seal_8_happy():
    with patch(
        "legacy_research.guards._seals_checks_6_10.check_seal_8_dependency_impl",
...
async def test_seal_9_happy():
    with patch(
        "legacy_research.guards._seals_checks_6_10.check_seal_9_compliance_impl",
        AsyncMock(return_value=(True, "verified")),
    ):
...
async def test_seal_10_happy():
    with patch(
        "legacy_research.guards._seals_checks_6_10.check_gate_21_preservation",
        AsyncMock(return_value=(True, "verified")),
    ):
