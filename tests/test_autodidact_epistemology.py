...
def test_epistemological_pipeline_valid():
    ...
    lat = compile_latent(
        inputs=[der],
        model=InferenceModel.BAYESIAN,
        infer_func=bayesian_infer
    )
    ...
    nar = Narrative(
        content="Release has been frozen due to high risk.",
        basis=inter
    )
