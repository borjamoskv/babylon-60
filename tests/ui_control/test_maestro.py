...
async def test_activate_app_success(maestro):
    with patch(
        "cortex.experimental.extensions.ui_control.maestro.run_applescript", new_callable=AsyncMock
    ) as mock_run:
...
async def test_inject_keystroke_app_not_running(maestro):
    with patch(
        "cortex.experimental.extensions.ui_control.maestro.is_app_running", new_callable=AsyncMock
    ) as mock_is_running:
