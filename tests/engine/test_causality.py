...
async def test_async_causal_graph_ops(tmp_path):
    ...
    async with aiosqlite.connect(db_path) as conn:
        ...
        await conn.execute("CREATE TABLE facts (id INTEGER PRIMARY KEY, confidence TEXT, tenant_id TEXT, metadata TEXT)")
...
async def test_propagate_taint_async(tmp_path):
    ...
    async with aiosqlite.connect(db_path) as conn:
        ...
        # Setup: 1 (source) -> 2 -> 3
        await conn.execute("INSERT INTO facts (id, confidence, tenant_id, metadata) VALUES (1, 'C5', 'default', '{}')")
        await conn.execute("INSERT INTO facts (id, confidence, tenant_id, metadata) VALUES (2, 'C5', 'default', '{}')")
        await conn.execute("INSERT INTO facts (id, confidence, tenant_id, metadata) VALUES (3, 'C5', 'default', '{}')")
        ...
        async with conn.execute("SELECT id, confidence, metadata FROM facts ORDER BY id") as cursor:
            ...
            # Verify audit edges
            async with conn.execute("SELECT COUNT(*) FROM causal_edges WHERE edge_type = ?", (EDGE_TAINTED_BY,)) as cursor:
