...
def test_pki_handshake_success(pki):
    ...
    message = f"{agent_id}:{bootstrap_token}".encode("utf-8")
...
def test_pki_handshake_invalid_signature(pki):
    ...
    message = f"fake_agent_id:{bootstrap_token}".encode("utf-8")
    sig_b64 = base64.b64encode(priv_key.sign(message)).decode("ascii")
...
def test_pki_revocation(pki):
    ...
    priv_key = ed25519.Ed25519PrivateKey.generate()
    pub_b64 = base64.b64encode(priv_key.public_key().public_bytes(
        encoding=serialization.Encoding.Raw,
        format=serialization.PublicFormat.Raw
    )).decode("ascii")
    ...
    message2 = f"{agent_id}:{new_token}".encode("utf-8")
