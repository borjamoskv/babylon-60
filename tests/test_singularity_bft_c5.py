...
async def test_vector_sqlite_wal_deadlock():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("SELECT COUNT(*) FROM security_audit_log WHERE action='DEADLOCK_TEST'")
...
async def test_vector_ledger_chain_break():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute("SELECT signature FROM security_audit_log WHERE audit_id = ?", (valid_hash,))
...
async def test_vector_swarm_apoptosis():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("SELECT COUNT(*) FROM security_audit_log WHERE action='SWARM_CONSENSUS'")
        count = (await cursor.fetchone())[0]
        assert count == 500
