...
current_import_parent: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "current_import_parent", default=None
)
...
class DivergenceReport(TypedDict):
    ...
    classification: Literal[
        "DIVERGENCE_IDENTITY", "DIVERGENCE_ORDER", "DIVERGENCE_LOADER", "DIVERGENCE_SIDE_EFFECT"
    ]
    details: str
...
class RuntimeCausalLedger:
    ...
    def record_event(
        self, kind: str, subject: str, inputs: dict[str, Any], outputs: dict[str, Any]
    ) -> str:
    ...
    def capture_snapshot(self) -> RuntimeSnapshot:
        ...
        import_cache_hashes = {k: repr(v) for k, v in sys.path_importer_cache.items()}
        ...
        if self.pre_snapshot and self.post_snapshot:
            if self.pre_snapshot["sys_meta_path_hash"] != self.post_snapshot["sys_meta_path_hash"]:
                ...
                mutations = [
                    ...
                    if e["kind"] == "mutation" and "sys.meta_path" in e["subject"]
                ]
                if not mutations:
                    return DivergenceReport(
                        severity="ERROR",
