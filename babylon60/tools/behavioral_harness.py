...
from babylon60.tools.system_identifier import SystemIdentifier, MahalanobisDistanceCalculator, BehavioralStateVector
from babylon60.tools.drift_detector import DriftDetector, DriftResult, SilentUpdateAlert
from babylon60.tools.coverage_entropy import CoverageAnalyzer
from babylon60.tools.excitation_batteries import BatteryManager, DifficultyLevel, ExcitationPrompt
...
class BehavioralHarness:
    ...
    def run_full_profile(
        ...
        robustness = self.analyzer.analyze_robustness(
            ...
            responses[1:4] if len(responses) > 1 else []
        )
        ...
        for fam in (families or ["L", "N", "M", "A", "Mc"]):
            fam_results = [r for r, p in zip(results, prompts) if p.family == fam]
            ...
            family_scores[fam] = {
                ...
                "avg_latency_ms": float(statistics.mean([r.latency_ms for r in success])) if success else 0.0
            }
    ...
    def compare_models(self, profile_a: BehavioralProfile, profile_b: BehavioralProfile) -> ComparisonReport:
        # Reconstruct trajectory classes
        traj_a = []
        for i, vec in enumerate(profile_a.state_trajectory):
            traj_a.append(BehavioralStateVector(
                turn_index=i, response_length=int(vec[1]*1000), lexical_entropy=vec[2],
                sim_to_context=vec[3], itl_ms=vec[4]*100, refusal_detected=bool(vec[5]),
        ...
        dtw_norm = self.sys_id.compute_trajectory_dtw_normalized(traj_a, traj_b, mahalanobis, window=10)
    ...
    def detect_drift(self, baseline: BehavioralProfile, current: BehavioralProfile) -> DriftReport:
        snap_a = self.drift_det.capture_snapshot(
            ...
            states=baseline.state_trajectory
        )
        snap_b = self.drift_det.capture_snapshot(
            model_id=current.model_id,
            states=current.state_trajectory
        )
