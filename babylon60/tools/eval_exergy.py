...
def calculate_eff(text: str) -> tuple[float, float]:
    ...
    struct_markers = len(re.findall(r'(^\s*[-*•]|\`\`\`|^\s*#\s|^\s*\[|^\s*\])', text, re.MULTILINE))
    struct_ratio = min(struct_markers / max(1, tokens * 0.1), 1.0)
    
    filler = len(re.findall(r'\b(solo que|básicamente|en resumen|por lo tanto|sin embargo|además)\b', text.lower()))
...
def main():
    ...
    text = sys.argv[1]
    ledger_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("babylon60/logs/exergy_ledger.json")
