...
VICTIM_KEYWORDS = re.compile(r'(censura|desmonetiza|quieren callar|cancelación|policía del pensamiento|ataque coordinado)', re.IGNORECASE)
CTA_KEYWORDS = re.compile(r'(patreon|apoya el canal|donación|cripto|enlace en la descripción|miembro|suscríbete para apoyar)', re.IGNORECASE)
...
def extract_vtt(url: str):
    ...
    cmd = [
        ...
        '--sub-format', 'vtt', '--output', 'temp_subs', url
    ]
    try:
...
def parse_vtt_and_analyze(vtt_file: str, video_id: str):
    ...
    if victim_timestamps and cta_timestamps:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?)
        ''', (video_id, f"Matches: {len(victim_timestamps)} victim keywords", "Agravio Estructural Detectado", True))
        conn.commit()
        print(f"[0x01_CORE] Disonancia extraída: Video {video_id}. Victim({len(victim_timestamps)}) -> CTA({len(cta_timestamps)})")
    else:
