...
logger = logging.getLogger("inject_semantic_bft")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para SEMANTIC-BFT...")
    ...
    md_path = "AUTODIDACT_SEMANTIC-BFT.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(BFT-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 120 # Elevated PID representation for Consensus Node
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "BFT_CONSENSUS_ROOT", 0.0)

    for i, match in enumerate(matches):
        p_id, p_name, p_app = [m.strip() for m in match]
        axiom_claim = f"{p_id}: {p_name}"
        constructive_proof = f"Consenso de estado verificado: {p_app}. Integrado en el grafo bizantino base-60."
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "BFT_QUORUM_NODE", 0.1)
    
    logger.info("Inyección criptográfica BFT completada.")
