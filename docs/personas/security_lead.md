...
If *any* of those four variables are altered directly in the database (e.g., an attacker gains access to `cortex.db` and changes `tenant_id` from `user_A` to `admin_1`), the hash chain fractures. The very next `verify()` call will trigger a system-wide breach alert.
...
The daemon actively scans incoming memory payloads for 11 distinct patterns (AWS IAM keys, Stripe tokens, private ECDSA keys). If detected, CORTEX refuses the write operation outright to prevent secret contamination in the episodic vector index.
