...
1.  **Identity Graph Gravity:** Every interaction refines the weight connections inside the spatial-temporal index (`cortex_memory_vsa.db`). Replacing BABYLON-60 with a naive database dump destroys years of contextual alignment.
2.  **Cross-App Identity Lock-In:** Once an agent's memory graph spans multiple independent applications, the user's digital identity resides in the BABYLON-60 layer.
