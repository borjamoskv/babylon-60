...
> leerse como clases o APIs implementadas dentro del árbol actual de BABYLON-60 Persist.
...
### 4. Bridge to BABYLON-60 (Recomendaciones 130/100)
- **Implementar Ledger-State**: BABYLON-60 debe adoptar el patrón de Hechos/Plan persistente en lugar de solo historial de chat para tareas largas.
...
- **Pre-procesado de contexto**: Si BABYLON-60 adopta una memoria que muta el contexto antes de invocar al modelo, debe anclarse en `cortex/memory/manager.py` (`CortexMemoryManager`) y en los hooks de búsqueda/recuperación del motor, no en una clase ficticia `CortexMemory`.
