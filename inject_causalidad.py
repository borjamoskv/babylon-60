...
logger = logging.getLogger("inject_causalidad")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas Causalidad...")
    ...
    md_path = "AUTODIDACT_CAUSALIDAD.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'\d+\.\s*\*\*(P\d{2,3})\s*-\s*([^\*:]+):\*\*\s*(.*)')
    matches = pattern.findall(content)

    agent_id = 99
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "CAUSALIDAD_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "CAUSALIDAD_LEAF", 0.1)
    
    logger.info(f"Completada inyección de {len(matches)} primitivas.")
