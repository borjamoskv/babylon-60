...
logger = logging.getLogger("inject_babylon60")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para BABYLON-60 Epistemology...")
    ...
    md_path = "AUTODIDACT_BABYLON-60.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(B60-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 100
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "BABYLON60_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "BABYLON60_LEAF", 0.1)
