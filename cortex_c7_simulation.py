...
def simulate_c7_economics(generations=100000):
    ...
    for gen in range(generations):
        ...
        if F_prod <= 0: P_prod *= 0.8; F_prod = 5.0
        if F_para <= 0: P_para *= 0.5; F_para = 5.0
        if F_hunt <= 0: P_hunt *= 0.8; F_hunt = 5.0
        if F_mal <= 0:  P_mal *= 0.5;  F_mal = 5.0
            
        # Umbrales de replicación (Fitness > 20)
        if F_prod > 20: P_prod *= 1.01; F_prod = 10.0
        if F_para > 20: P_para *= 1.05; F_para = 10.0
        if F_hunt > 20: P_hunt *= 1.02; F_hunt = 10.0
        if F_mal > 20:  P_mal *= 1.05;  F_mal = 10.0
