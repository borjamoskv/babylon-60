...
try:
    from db import log_scaffold_experiment, get_failed_experiments  # type: ignore
except ImportError:
...
def semantic_compressor(state: AgentState):
    ...
    print(f"\n[◈ COMPRESSOR] Ejecutando Attention Matching sobre el código fuente...")
    compressor = ContextCompressor()
    
    # Compresión al 60% por defecto
    compressed = compressor.compress_code(state["target_code"], ratio=0.6)
    
    orig_size = len(state["target_code"])
    new_size = len(compressed)
    reduction = (1 - new_size / orig_size) * 100
    
    print(f"[◈ COMPRESSOR] Reducción: {reduction:.1f}% ({orig_size} -> {new_size} bytes)")
    
    return {
        "compressed_code": compressed,
        "metadata": {**state.get("metadata", {}), "original_code_size": orig_size, "compression_ratio": 0.6}
    }
