...
logger = logging.getLogger("inject_thermo_compression")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para THERMO-COMPRESSION...")
    ...
    md_path = "AUTODIDACT_THERMO-COMPRESSION.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(THC-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 100
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "THERMO_COMPRESSION_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "THERMO_COMPRESSION_LEAF", 0.1)
